...
%An excess of \mumu events is searched for over a range
%is searched for in the dimuon
%component, consistent with \dbtomumu.
%The mass and the lifetime of the \db is not assumed anywhere in the analysis.
%%in the dimuon component
%%of the decay \btokstrmumu, consistent with
...
%The inclusion of a dark sector naturally provides a \dm candidate
...
The interaction terms in the Lagrangian for these portals are shown in \Table{tab:db:overview}, and
the following paragraphs will go into further detail about the Higgs and vector portals.

%These messenger particles could interact via a number of \emph{portals}, which include the vector,
...
%There are a number of experimental observations, as outlined in \Chap{ch:th} that indicate that the
%\sm is an incomplete pictures of particle physics.
%A possible extension to the \sm is to introduce a dark sector, which can contain a spectrum of
%particles beolonging to a differeny gauge group the \sm particles.
%The inclusion of a dark sector naturally provides a \dm candidate, and can solve other problems,
%depending on the exact model.
%These dark particles typically cannot interact with any of the \sm particles, howeever most models
%include a messenger particle which can feebly couple to the known spectrum of particles.
%Messenger particles can interact with the \sm particles through one of four main \emph{portals}:
%the vector, axion, Higgs, and neutrino portals.
%The interaction terms in the Lagrangian for these portals are shown in \Table{tab:db:overview}, and
%the following paragraphs will go into further detail about the Higgs and vector portals.
...
\begin{figure}
  \begin{center}
    \includegraphics[width=0.48\textwidth]{A-visible-large}
    \includegraphics[width=0.48\textwidth]{A-visible-zoom}
    \caption{
...
\decay{\Bp}{\Kp\db} where \db is a scalar and an pseudoscalar particle; the decay rate predictions
