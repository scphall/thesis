...
The size of the deviation is dependent upon the width of a resonance, $\Gamma$, its magnitude
...
will be vetoed, while those with $\Gamma>125\mev$ shall be ignored.
...
$\Gamma\sim70\mev$, and are dealt with by reducing the size of the sidebands, by setting $x=1$ and
...
\btokstrmumu and \decay{\Bd}{\Kstarz\rho}, where \decay{\rho}{\mumu}.
Despite the fact that the total branching fractions of the $\rho$ decay is so small that it is
expected that less than one event will remain in the final data sample, the cross-term in the
interference could result in a sizable deviation from local-linearity.
However, this deviation can only be a maximum of \approx$5\pc$, which is accounted for in the choice
of $\sigma_y$.
...
model~\cite{Bobeth:2011nj} which can account for the interference effects between the
\decay{\ccbar}{\mumu} resonances and the non-resonant \mumu component.
The model is used to generate \glspl{PDF} with different phases in order to find the maximum
possible deviation from non-linearity in the \mumu spectrum.
...
background is locally linear to better than $5\pc$ for various values of $x$; examples of them are
shown in \Fig{fig:db:ccbar}.
...
Sensitivity to \lifetime{\db} is introduced by splitting the data at each \mass{t} into two
...
\begin{align}
  ...
  \label{eq:db:liketau}
\end{align}
In these expressions the superscripts $p$ and $d$ indicate the prompt and displaced regions,
respectively.
...
Therefore the background shape information is not used.


\subsection[Calculation of $p$-value]
{Calculation of $\boldsymbol{p}$-value}
At each \mass{t}, a $p$-value is calculated using profile likelihood of the joint likelihood given
in \Eq{eq:db:liketau}.
