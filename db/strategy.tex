...
The joint likelihood is the product of the two individual likelihoods:
\begin{align}
  ...
  \label{eq:db:liketau}
\end{align}
where superscript $p(d)$ denotes the prompt(displaced) region.
...
\end{equation}
Then, $-2\ln\big(\Lambda_\mathrm{Tot}\big)$ behaves as a \chisq distribution with two
degrees of freedom.
Since this is analytically solvable, a $p$-value can be calculated
quantifying how incompatible the number of events observed in the signal region is with the null
hypothesis of zero signal events.
...
The \emph{look-elsewhere effect} is the phenomenon whereby an experiment yields a
...
is observed will be isolated and removed from the sample; leaving data
which is entirely background-like.
This is assuming that there is only one \np particle to be observed in this analysis.
The remaining background-like distribution is turned into a \PDF, where the region that has
been removed is interpolated across.
Toy datasets can then be generated from this \PDF.
%This procedure is described more fully later.

This method requires the generation of $\mathcal{O}(10^7)$ datasets to probe down to a
$5\sigma$ global $p$-value.
If the minimum local $p$-value observed is less than any of the local $p$-values in the toy
datasets then the asymptotic formula will be used~\cite{Gross:2010qma}.
...
\begin{table}
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption[Summary of veto regions for \qqbar states]
  {
