...
respectively.
The combined profile likelihood is then the product of the profile likelihoods for the prompt and
displaced regions in decay time
\begin{equation}
  \Lambda_\mathrm{Tot}
  =
...
\end{equation}
Asymptotically, $-2\ln\big(\Lambda_\mathrm{Tot}\big)$ behaves as a \chisq distribution with two
degrees of freedom.
...
As shown in \Ref{Davies:1987zz}, this bound is given by
\begin{equation}
...
The function $N(c)$ gives the number of \emph{upcrossings} of a dataset above the level of $c$,
where an upcrossing is defined as the point where the $p$-value crosses from below the value of
$c$, to above it.
...
dependent on the datasets used, and therefore an approximation is used.
Reference~\ref{Gross:2010qma} shows that \Eq{eq:db:asy1} can be approximated to be
\begin{equation}
  P_\mathrm{global} \leq P(\chisq_s > c)
  + \langle N(c)\rangle\left(\frac{c}{c_0}\right)^{\frac12(s-1)}\exp\left(-\frac{c-c_0}{2}\right).
  \label{eq:db:asy2}
\end{equation}
Therefore, once $\langle N(c)\rangle$ has been determined using a few hundred toy datasets, the
