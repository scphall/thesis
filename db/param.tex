...
Once the two-dimensional map of $\mathcal{T}(m,\tau)$ has been produced, it is scaled such that at
...
a \db --- as a function of \qsq, and the rescaled distribution of $\mathcal{T}(m,\tau)$, which shall
...
To summarize, it is the lifetime distribution for a dark boson where $\tau_\db=100\ps$, which has
been scaled such that at $\tau=0$ the distribution is equal to
...
into account by convolving $\mathcal{R}(m,\tau)$ with a decaying exponential to derive an efficiency
map:
\begin{equation}
  \varepsilon(m,\tau) =
