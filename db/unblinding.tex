...
This search for a new dark boson is done blindly to ensure that the analysis is not biased in the
...
\Ref{LHCb-CONF-2015-002}, yields $320$ thousand candidate decays.
Using the selection from \Sect{sec:db:sel} and approximately $\sim290$ thousand
...
To determine these efficiencies, we apply the pre-BDT selection and perform a fit to determine the
$B$ yield.  Next, we apply the BDT cut and perform a second fit, then take the ratio of the $B$
yields, this comparison is shown in \Fig{fig:bdtEffRatio}.
Data and simulation are shown to be in good agreement, centred around unity with about 5--10\%
precision.
...
Given that we explicitly
veto all peaking backgrounds that are visible in the various 2-body mass combinations, we do not
expect any peaking background contributions at the level of the statistical precision of this test.
...
From these distributions toy datasets can be generated to be used to convert the local minimum
...
and \lifetime{\db}.
If the global $p$-value corresponds to being greater than $\sigma$, then the excess will be fit to
determine the best-fit mass and lifetime.
