...
%To ensure that the
Selection criteria are summarised in \Tab{tab:dsphi:sel}.
...
\begin{table}[!ht]
  ...
  {
    Stripping selection criteria applied to the \btodsphi candidates.
...
%These peaking backgrounds do not necessarily result in a longitudinally polarised \phii.
%They are however, irreducible, and must be accounted for in the fit, this is described later in
%\Sec{sec:dsphi:fit}.
...
sideband of the \Ds.
\added{
  The upper mass sideband is guaranteed to be pure combinatorics with some misidentified \Lc and
  \Dp decays, which are removed with efficient vetoes, as described in \Sect{sec:dsphi:sel:veto}.
}
...
has been identified as an $i$ --- \added{for example, a particle identified as a kaon but under the
