...
becuase $\psi_L$ and $\psi_R$ have different $U(1)$ charges, %Y hypercharge
...
\end{align}
However, it is more convenient to change a basis in which the matrix $m^q$ is diagonal, using the
rotation matrices $V_L$ and $V_R$
\begin{align}
...
$_{L}^{q\dagger} V_{L}^{q} = V_{R}^{q\dagger} V_{R}^{q} = \mathbb{1}$.
...
\begin{align}
  ...
  &= i\frac{g}{2}\gamma^\mu
  \bigg[
    \bar u_L\left(V_{uL}^{\phantom{\dagger}}V_{dL}^\dagger\right)d_LW^+_\mu +
    \bar d_L\left(V_{dL}^{\phantom{\dagger}}V_{uL}^\dagger\right)u_LW^-_\mu
  \bigg]  \nonumber\\
  &= i\frac{g}{2}\gamma^\mu
  \bigg[
    \VCKM\bar u_Ld_LW^+_\mu +
    \VCKMconj\bar d_Lu_LW^-_\mu
  \bigg].
\end{align}
The matrix $\VCKM=\V{uL}V_{dL}^\dagger$ is defined is known as the Cabibbo-Kobayashi-Maskawa (CKM)
