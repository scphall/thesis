...
%goldstino
Supersymmetry (SUSY) is a popular theory which solves a host of problems with the SM.
It introduces a new spectrum of particles, each of which mirrors a SM particle but whose spin
differs by half.
This new symmetry immediately solves the hierarchy problem and supplies a Dark Matter candidate in
the shape of the lightest supersymmetric particles.
...
New physics can cause significant deviations in physical observables with respect to SM
predictions.
These differences are most noticeable in processes which are highly suppressed in the SM, or can
have NP contributions at first order.
...
boson, such as an $H^+$ from SUSY, which could significantly alter the branching fraction or
...
The \lhcb experiment can probe the mass scale, since precision measurements are sensitive to
virtual poparticles contributing at all orders, whose on-shell mass could be many TeV.
The relatively high luminosity of interactions supplied by the \lhc mean that \lhcb is also
sensitive to low coupling strengths.
