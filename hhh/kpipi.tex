...
\subsubsection{Branching fracion of normalization channel}
The branching fraction of the decay \btopsitwosk is~\cite{PDG2012}
\begin{equation}
  \BF_\mathrm{tot}\big(\btopsitwosk\big)
  =\big(1.264\pm0.052\big)\e{-5}.
\end{equation}
The uncertainty on this is fully correlated between all \qsq bins.


\subsubsection{Corrections to simulation}
%\subsubsection{Tracking effectiency}
Estimated by redetermining relative efficienices without the tracking efficiency correction
%\subsubsection{PID efficiency}
Without using resampling
%\subsubsection{IsMuon and PIDmu requiremene}
Without
%\subsubsection{Reweighting of variables}
Without


\subsubsection{Modelling of decay}
%\subsubsection{\qsq model}
%\subsubsection{Kpipi composistion}
%\subsubsection{Angle \thetakone}
%\subsubsection{Charmonium vetoes}
Multiple MC samples at generator level, each with different form factors (assumed to be
uncorrelated)
Each FF is chosen from a number folllowing gaussian distribution with mean of central value and
std of the err on the FF parameters.


\subsubsection{Fits}
%\subsubsection{Signal mass model}
Single, not double, CB
%\subsubsection{Background mass model}
linear
