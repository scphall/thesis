...
squared.
This analysis was published in \Ref{LHCb-PAPER-2014-030}.
%Figure~\ref{fig:hhh:feyn} shows the penguin diagrams which propagate these decays in the SM.
...
\begin{figure}
  \begin{center}
    ...
    \caption[Schematic Feynman diagrams for FCNCs]
    {
      ...
      box diagram.
      Operator \Op7 describes the photonic penguin diagram; while \Op9 and \Op{10} are the
...
%The operators \Op7 and \Op9 describe the \fcnc via vec-or and axial vector transitions.
...
%like a penguin.
Figure~\ref{fig:hhh:loops} shows schematic Feynman diagrams for \Op7, \Op9, and \Op{10}.
...
loops, and \Op8 is the gluonic penguin operator.
...
prediction.
%The decay \decay{\Bd}{\Kp}{\mumu} is also stud
%~\cite{LHCb-2014-007}hav.
...
%The measured value of $P_5^\prime$ is high with respect to the standard model prediction,
%indicating that $C_9$ is low~\cite{Altmannshofer:2014rta} with respect to standard model
%predictions, this could indicate a new vector contribution.
...
\begin{figure}
  \begin{center}
    \includegraphics[scale=1,valign=t]{feynman_hhh_kpipimumu}
    \includegraphics[scale=1,valign=t]{feynman_hhh_phikmumu}
...
The total branching fraction of the decay \decay{\kone{1270}}{\kpipi}
...
\begin{figure}
  \begin{center}
    \begin{tikzpicture}
      \coordinate[] (p1);
      \coordinate[above=4.5 of p1] (p2);
      \coordinate[left=1 of p2] (p3);
      \coordinate[right=6 of p3] (p4);
      \draw (p1) node {\includegraphics[width=1.00\textwidth]{K1_q2_theory}};
      \draw (p3) node {\includegraphics[scale=0.65]{legend_red}};
      \draw (p4) node {\includegraphics[scale=0.65]{legend_blue}};
    \end{tikzpicture}
...
\begin{equation}
  ...
  5.80\,^{+0.6}_{-0.5}.
\end{equation}
%Also, very approximately, a photonic penguin is suppressed with respect to a gluonic penguin by a
%factor of $(4\pi\alpha)^2\simeq10^{-2}$.
%From these numbers it should be expected that $\BF(\btokpipimumu)\simeq5\e{-7}$ and
%$\BF(\btophikmumu)\simeq0.8\e{-8}$.
