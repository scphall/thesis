...
\begin{equation}
  \Ham{eff} = -4\frac{G_F}{\sqrt{2}}\Vconj{ts}\V{tb}
  \sum_{i}\big(c_i(\mu)\mathcal{O}_i(\mu) + c_i(\mu)^\prime\mathcal{O}_i^\prime(\mu)\big)
\end{equation}
similar to \Eq{eq:th:lageff}, where the opertors are known as Wilson operators.
Each Wilson operator describes a different physical process.
The Wilson operators, $\mathcal{O}_{1-6}$ are
